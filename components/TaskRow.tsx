...
const TaskRow: React.FC<TaskRowProps> = ({ task, onEditTask, onDeleteTask, isLast }) => {
  ...
  return (
    <tr className={`hover:bg-slate-750 transition-colors ${isLast ? '' : 'border-b border-slate-700'}`}>
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-200">{task.title}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm">
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[task.status] || 'bg-gray-500 text-gray-100'}`}>
          {task.status}
        </span>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm">
